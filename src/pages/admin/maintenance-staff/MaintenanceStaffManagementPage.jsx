...
/* ------------------------------------------------------------------
   ...
   ------------------------------------------------------------------
   Same pattern as HallAuthorityManagementPage, filtered to role =
   "Maintenance Staff". These accounts aren't tied to a single hall
   in the mock data, so showHallColumn is left off UserTable.
------------------------------------------------------------------- */
...
export default function MaintenanceStaffManagementPage() {
  const [users, setUsers] = useState(initialUsers.filter((u) => u.role === "Maintenance Staff"));
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Maintenance Staff Management</h1>
          <p className="text-sm text-slate-500">Manage maintenance staff accounts — work order assignment is handled by Hall Authorities</p>
        </div>
        <button className="flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700">
          <UserPlus className="h-4 w-4" /> Create Maintenance Staff
        </button>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white">
        {paginated.length === 0 ? (
          <EmptyState title="No maintenance staff found" message="Try adjusting your search or filters." />
        ) : (
          <UserTable
            users={paginated}
      ...
      <ConfirmDialog
        open={!!confirmAction}
        title={confirmAction?.type === "delete" ? "Delete this staff account?" : `${confirmAction?.user?.status === "Active" ? "Deactivate" : "Activate"} this account?`}
        message={
