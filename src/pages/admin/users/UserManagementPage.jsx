...
/* ------------------------------------------------------------------
   HallMate — User Management (FRONTEND-ONLY)
   ------------------------------------------------------------------
   Reuses the common components already built for the student side
   (SearchBar, FilterPanel, Pagination, ConfirmDialog, EmptyState) so
   nothing is redefined here — only UserTable and the badges are new,
   and those are shared too (used again by Hall Authority Management
   and Maintenance Staff Management in a later step).
------------------------------------------------------------------- */
...
export default function UserManagementPage() {
  ...
  const handleFilterChange = (key, value) => { setFilterValues((p) => ({ ...p, [key]: value })); setPage(1); };
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Manage students, hall authorities, maintenance staff, and admins</p>
        </div>
      ...
      <ConfirmDialog
        open={!!confirmAction}
        title={confirmAction?.type === "delete" ? "Delete this user?" : `${confirmAction?.user?.status === "Active" ? "Deactivate" : "Activate"} this user?`}
        message={
